...
try:
    ...
    # CONTROL CRÍTICO: Si los datos vienen vacíos, mostramos advertencia limpia en vez de crashear
    if df_orders.empty:
        st.warning("⚠️ No se pudieron cargar las órdenes. Revisá la configuración de las credenciales o que las tablas en Google Sheets no estén vacías.")
    else:
        # Limpiar columnas de forma segura
        df_orders = clean_dataframe_columns(df_orders)
        
        # Forzar tipo string en columnas clave para evitar errores de .str accessor
        ...
        with col1:
            if not df_partners.empty and 'nombre' in df_partners.columns:
                ...
                st.caption("Filtro de clientes no disponible (tabla vacía)")
